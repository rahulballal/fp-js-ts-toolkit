...
const getTotalPriceValue = ({ totalPrices }) => {
	...
	return total.totalPriceValue === '' ? null : parseFloat(total.totalPriceValue)
}
...
export default function flagEqualLowestBids(bids = []) {
	...
	const [otherBidsTiedForLowest, otherBidsNotTiedForLowest] = partition(otherBids, (bid) => {
		return (
			getTotalPriceValue(bid) === getTotalPriceValue(lowestBid) && !!bid.rank && !!lowestBid.rank
		)
	})
