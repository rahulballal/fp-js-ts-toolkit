...
const isPriceCreatedBefore = (createdAt, newCreatedAt) =>
  moment(createdAt).isBefore(newCreatedAt);

// Now a very simple Unit test
const totalPricesReducer = (accum, price) => {
  if (
    !R.isEmpty(accum) ||
    isPriceCreatedBefore(accum.createdAt, price.createdAt)
  ) {
    accum = price;
  }
  return accum;
...
const getTotalPriceValue = ({ totalPrices = [] }) =>
  R.pipe(
    ...
    ({ totalPriceValue }) =>
      R.isEmpty(totalPriceValue) ? null : parseFloat(totalPriceValue) // Step 3
  )(totalPrices);
...
const bidPartitionPredicate = (lowestBid) => (aBid) =>
  R.allPass([
    ({ lowestBid, aBid }) =>
      R.equals(getTotalPriceValue(lowestBid), getTotalPriceValue(aBid)),
...
const flagEqualLowestBids = (bids = []) => {
  ...
  const equalLowestBidFlagged = R.map(
    R.mergeRight({ tiedForLowest: true }),
    equalLowestBids
  );
